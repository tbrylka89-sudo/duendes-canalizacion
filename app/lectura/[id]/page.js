...
// ═══════════════════════════════════════════════════════════════════════════════
// PAGINA DE LECTURA - CONTENIDO PRIVADO
// Lecturas ancestrales personalizadas (noindex)
// ═══════════════════════════════════════════════════════════════════════════════

/**
 * Genera metadata dinamico para cada lectura
 * IMPORTANTE: noindex para proteger contenido privado
 */
export async function generateMetadata({ params }) {
  const { id } = await params;

  // Intentar obtener el nombre del cliente para el titulo
  let nombreCliente = 'tu';
  try {
    const lectura = await kv.get('lectura:' + id);
    if (lectura?.nombreCliente) {
  ...
  return {
    ...generatePrivateMetadata(
      `Lectura Ancestral para ${nombreCliente}`,
      'Tu lectura ancestral personalizada de Duendes del Uruguay. Contenido privado y unico.'
    ),
...
export default async function LecturaPage({ params }) {
  ...
  if (!lectura) {
    return (
      <div style={{
        ...
        <h1 style={{ color: '#4A5D4A' }}>Lectura no encontrada</h1>
        <p style={{ color: '#666', marginTop: '20px' }}>
          Esta lectura no existe o el enlace ha expirado.
        </p>
        <a
  ...
  const Section = ({ children, dark, accent }) => (
    <section style={{
      background: dark ? '#4A5D4A' : accent ? '#f9f6f0' : 'white',
      ...
      fontSize: '16px',
      whiteSpace: 'pre-wrap',
      border: accent ? '2px solid #C6A962' : 'none'
    }}>
      {children}
    </section>
  );
    ...
    <div style={{ fontFamily: 'Georgia', background: '#FDF8F0', minHeight: '100vh', padding: '20px' }}>
      <div style={{ maxWidth: '800px', margin: '0 auto' }}>
        ...
        <div style={{
          ...
          <p style={{ color: '#4A5D4A', fontSize: '18px', marginBottom: '20px' }}>
            Queres un guardian canalizado especialmente para vos?
          </p>
          <a
            href="https://duendesdeluruguay.com"
            style={{
