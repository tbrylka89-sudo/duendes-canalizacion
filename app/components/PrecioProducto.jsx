...
const PRECIOS_URUGUAY = {
  convertir: (precioUSD) => {
    ...
    if (precio <= 75) return 2500;      // Mini clásico
    if (precio <= 160) return 5500;     // Mini especial / Pixie
    if (precio <= 210) return 8000;     // Mediano especial
    if (precio <= 350) return 12500;    // Mediano maestro
    if (precio <= 500) return 16500;    // Grande especial
    if (precio <= 700) return 24500;    // Grande maestro
    if (precio <= 1100) return 39800;   // Gigante especial
...
export default function PrecioProducto({ precioUSD, precioRegular, enOferta, className = '' }) {
  ...
  useEffect(() => {
    ...
    if (paisCookie === 'UY') {
      ...
      if (enOferta && regular) {
        const regularUYU = PRECIOS_URUGUAY.convertir(regular);
        setPrecioRegularFormateado(`$${formatearNumero(regularUYU)} UYU`);
      }
      ...
      // Otros países: USD + aproximación en moneda local
      ...
      if (monedaInfo && monedaInfo.codigo !== 'USD') {
        ...
        if (enOferta && regular) {
          ...
          setPrecioRegularFormateado(
            `$${formatearNumero(regular)} USD (aprox. ${monedaInfo.simbolo}${formatearNumero(regularLocal)})`
          );
        }
      } else {
        // Default: solo USD
  ...
  // Mientras carga, mostrar precio en USD (SSR fallback)
  if (!precioFormateado) {
    return (
