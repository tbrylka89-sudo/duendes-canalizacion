...
add_action('rest_api_init', function () {
    ...
    // Endpoint para subir plugins
    register_rest_route('duendes/v1', '/upload-plugin', [
        'methods' => 'POST',
        'callback' => 'duendes_upload_plugin_handler',
        'permission_callback' => 'duendes_verify_secret'
    ]);

    // Endpoint para activar plugins
    register_rest_route('duendes/v1', '/activate-plugin', [
        'methods' => 'POST',
        'callback' => 'duendes_activate_plugin_handler',
        'permission_callback' => 'duendes_verify_secret'
    ]);
});
...
/**
 * Handler para subir plugins
 */
function duendes_upload_plugin_handler($request) {
    $filename = sanitize_file_name($request->get_param('filename'));
    $content = $request->get_param('content');
    $from_url = $request->get_param('url');

    if (!$filename) {
        return new WP_REST_Response(['success' => false, 'error' => 'filename requerido'], 400);
    }

    // Si viene URL, descargar de ahí
    if ($from_url) {
        $content = @file_get_contents($from_url);
        if (!$content) {
            // Intentar con wp_remote_get
            $response = wp_remote_get($from_url, ['timeout' => 30]);
            if (!is_wp_error($response)) {
                $content = wp_remote_retrieve_body($response);
            }
        }
        if (!$content) {
            return new WP_REST_Response(['success' => false, 'error' => 'No se pudo descargar desde URL'], 400);
        }
    } elseif ($content) {
        // Si viene content en base64, decodificar
        $decoded = base64_decode($content);
        if ($decoded) {
            $content = $decoded;
        }
    } else {
        return new WP_REST_Response(['success' => false, 'error' => 'content o url requerido'], 400);
    }
...
/**
 * Handler para activar plugins
 */
function duendes_activate_plugin_handler($request) {
    $plugin = $request->get_param('plugin');

    if (!$plugin) {
        return new WP_REST_Response(['success' => false, 'error' => 'plugin requerido'], 400);
    }
