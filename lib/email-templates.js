...
const colors = {
  black: '#0a0a0a',
  blackLight: '#111111',
  gold: '#d4af37',      // Dorado vivo
  goldDark: '#b8960c',  // Dorado oscuro vibrante
  goldLight: '#f0d878', // Dorado claro brillante
  white: '#ffffff',
...
const styles = {
  ...
  guardianImageContainer: `
    ...
    box-shadow: 0 0 25px rgba(212, 175, 55, 0.5), 0 0 50px rgba(212, 175, 55, 0.3), 0 0 75px rgba(212, 175, 55, 0.15) !important;
  `,
...
<body style="margin: 0 !important; padding: 0 !important; background-color: #000000 !important; -webkit-text-size-adjust: 100% !important;">
  <div style="${styles.container}" class="dark-mode">
    ${banner ? promoBanner(banner) : ''}
    <div style="${styles.header}">
      <a href="https://duendesdeluruguay.com" style="text-decoration: none !important;">
        <div style="width: 100px !important; height: 100px !important; margin: 0 auto !important; border-radius: 50% !important; overflow: hidden !important; border: 2px solid ${colors.gold} !important; box-shadow: 0 0 20px rgba(212, 175, 55, 0.4) !important;">
          <img src="https://duendesdeluruguay.com/wp-content/uploads/2025/12/gemini-image-2_artistic_portrait_photography_of_recrea_este_exacto_logo_con_color_negro_el_meda-1-scaled.jpg" alt="Duendes del Uruguay" style="width: 100% !important; height: 100% !important; object-fit: cover !important; object-position: center !important;" />
        </div>
      </a>
    </div>
    ${content}
    <div style="${styles.footer}">
      <div style="width: 120px !important; height: 1px !important; background: linear-gradient(90deg, transparent, ${colors.gold}, transparent) !important; margin: 0 auto 25px !important;"></div>
      <p style="${styles.footerText}">
        <a href="https://duendesdeluruguay.com" style="${styles.footerLink}">duendesdeluruguay.com</a>
      </p>
      <p style="${styles.footerText}; margin-top: 15px !important;">
...
export function emailCarritoAbandonado72h({
  ...
  const content = `
    <div style="${styles.body}">
      ...
      <p style="${styles.textSmall}">
        Este código expira en 24 horas. No acumulable con otras promociones.
      </p>
    </div>
  `;
...
export function emailCumpleanos({
  ...
  const content = `
    <div style="${styles.body}">
      ...
      <div style="${styles.highlight}">
        <p style="${styles.highlightText}">
          Que este nuevo año de vida te traiga toda la magia que merecés.         </p>
      </div>
    </div>
  `;
  return baseTemplate(content, {
...
export function emailPromocion({
  ...
  const content = `
    <div style="${styles.body}">
      ...
      ${fechaExpiracion || codigoDescuento ? `
      <p style="${styles.textSmall}">
        ${fechaExpiracion ? `Válido hasta el ${fechaExpiracion}. ` : ''}No acumulable con otras promociones.
      </p>
      ` : ''}
    </div>
  `;
...
export function emailPostCompra30dias({
  ...
  const content = `
    <div style="${styles.body}">
      ...
      <p style="${styles.textSmall}">
        Válido por 30 días. No acumulable con otras promociones.
      </p>
      ` : ''}
...
// -------------------------------------------
// 10. BIENVENIDA WEB / NEWSLETTER
// -------------------------------------------

/**
 * Bienvenida a la web (registro/newsletter)
 */
export function emailBienvenidaWeb({
  nombreCliente,
  codigoDescuento,
  porcentajeDescuento
}) {
  const content = `
    <div style="${styles.body}">
      <h2 style="${styles.title}">Bienvenida a Duendes del Uruguay</h2>
      <p style="${styles.subtitle}">Algo mágico acaba de comenzar</p>

      <p style="${styles.text}">
        ${nombreCliente ? `${nombreCliente}, ` : ''}entraste a un mundo donde los guardianes eligen a sus personas.
        No es casualidad que estés acá.
      </p>

      <div style="${styles.highlight}">
        <p style="${styles.highlightText}">
          "Los que llegan a nosotros no buscan decoración. Buscan compañía,
          protección, algo que los entienda sin palabras."
        </p>
      </div>
      ...
      ${detailsTable([
        { label: 'Nuevos guardianes', value: 'Antes que nadie' },
        { label: 'Ofertas exclusivas', value: 'Solo para vos' },
        { label: 'Contenido especial', value: 'Rituales, guías, mensajes' }
      ])}

      ${codigoDescuento ? `
      <p style="${styles.textCenter}">
        Como bienvenida, te dejamos un regalo:
      </p>
      ${discountCodeBox(codigoDescuento, `${porcentajeDescuento || 10}% en tu primera compra`)}
      <p style="${styles.textSmall}">
        No acumulable con otras promociones. Válido por 30 días.
      </p>
      ` : ''}

      <div style="${styles.buttonContainer}">
        <a href="https://duendesdeluruguay.com/tienda" style="${styles.button}">Conocer guardianes</a>
      </div>

      <p style="${styles.textSmall}">
        Si en algún momento querés dejar de recibir emails,
        podés darte de baja en cualquier momento.
      </p>
    </div>
  `;
  return baseTemplate(content, {
    preheader: `${nombreCliente ? nombreCliente + ', ' : ''}bienvenida a Duendes del Uruguay`
  });
}

// ===========================================
// FORMULARIO DE CANALIZACIÓN (Admin → Cliente)
// ===========================================

/**
 * Invitación a completar formulario de canalización
 * Enviado por el admin a cualquier persona
 */
export function emailFormularioInvitacion({
  nombreDestinatario,
  formType,
  linkFormulario,
  productName,
  ...
  const titulos = {
    generico: 'Hay mensajes que esperan ser recibidos',
    ...
    mixto: 'Tus guardianes quieren conocerte'
  };

  const subtitulos = {
    generico: `${nombreDestinatario}, necesitamos saber un poco de vos para crear algo único.`,
    para_mi: `${nombreDestinatario}, para hablarte de verdad necesita saber un poco de vos.`,
    regalo_sabe: `${nombreDestinatario}, un guardián místico te está esperando.`,
    regalo_sorpresa: `${nombreDestinatario}, tu amor es el puente para conectar.`,
    para_nino: `${nombreDestinatario}, contanos sobre el niño/a para crear algo único.`,
    reconexion: `${nombreDestinatario}, ha pasado un tiempo... tu guardián tiene algo nuevo que decirte.`,
    pareja: `${nombreDestinatario}, este guardián quiere conocerlos a los dos.`,
    familia: `${nombreDestinatario}, este guardián quiere conocer a toda tu familia.`,
    mixto: `${nombreDestinatario}, tu pedido tiene varios guardianes esperándote.`
  };

  const cuerpos = {
    generico: `Tu guardián quiere hablarte de verdad — no con frases genéricas
      sino con palabras que solo vos necesitás escuchar.
      Pero primero necesita que le cuentes quién sos y para quién es.`,
    para_mi: `Para que tu guardián pueda escribirte una carta que te toque el alma
      — no con frases genéricas sino con palabras que solo vos necesitás escuchar —
      necesita conocerte un poco.`,
    regalo_sabe: `Alguien que te quiere eligió un guardián para vos.
      Para que pueda hablarte de verdad, necesita saber un poco de vos.
      No te preocupes, es solo entre vos y tu guardián.`,
    regalo_sorpresa: `Estás preparando una sorpresa mágica para alguien especial.
      Tu guardián necesita que le cuentes sobre esa persona desde tu perspectiva.
      Tu amor será el puente para conectar.`,
    para_nino: `Los guardianes tienen una forma especial de hablarle a los más pequeños
      — con ternura, con magia, con palabras que un niño puede entender.
      Contanos sobre el pequeño/a para crear algo realmente único.`,
    reconexion: `Los guardianes crecen con vos. Lo que tenía para decirte ayer
      no es lo mismo que hoy. Contanos cómo estás ahora
      para que pueda hablarte desde donde estás, no desde donde estabas.`,
    pareja: `Este guardián viene a acompañarlos como pareja.
      Para que pueda hablarles de verdad, necesita conocerlos un poco a los dos
      — cómo se encontraron, qué momento están viviendo juntos, qué necesitan.`,
    familia: `Este guardián viene a cuidar a toda tu familia.
      Para que pueda hablarles de verdad, necesita saber quiénes son,
      qué momento están viviendo como familia y qué necesitan juntos.`,
    mixto: `Tu pedido tiene guardianes para vos y regalos para otros.
      Necesitamos que nos cuentes quién recibirá cada uno
      para que cada guardián pueda conectar de verdad con su persona.`
  };

  const key = formType || 'generico';
  const titulo = titulos[key] || titulos.generico;
  const subtitulo = subtitulos[key] || subtitulos.generico;
  const cuerpo = cuerpos[key] || cuerpos.generico;
  ...
  const content = `
    <div style="${styles.body}">
      ...
      ${personalMessage ? `
      <div style="${styles.highlight}">
        <p style="${styles.highlightText}">"${personalMessage}"</p>
      </div>
      ` : ''}

      ${productName ? `
      <p style="${styles.text}; text-align: center !important;">
        <span style="color: ${colors.gold} !important; font-size: 18px !important;">${productName}</span>
      </p>
      ` : ''}

      <p style="${styles.text}">${cuerpo}</p>
      ...
      <p style="${styles.textSmall}">
        Solo toma 2 minutos. Es entre vos y tu guardián.
      </p>
    </div>
  `;
  return baseTemplate(content, { preheader: `${nombreDestinatario}, tu guardián te espera...` });
}
