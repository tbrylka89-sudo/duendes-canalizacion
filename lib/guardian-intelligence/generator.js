 ...
 */
export function seleccionarHookApertura(categoria, hooksUsados = []) {
  ...
  if (disponibles.length === 0) {
    return hooksCategoria[Math.floor(Math.random() * hooksCategoria.length)];
  }
...
export async function generarHistoriaUnica(datosGuardian, contexto = {}) {
  ...
  const hookApertura = seleccionarHookApertura(categoria, hooksUsados);
...
📌 VARIEDAD EN APERTURAS - MUY IMPORTANTE:
- Cada historia debe empezar DIFERENTE a las anteriores
- Si la última usó "¿Cuántas veces...?", esta NO puede usar esa estructura
- Podés usar: afirmaciones directas, reflexiones, descripciones, preguntas retóricas VARIADAS
- El objetivo es que al leer 5 historias seguidas, ninguna empiece igual
...
export async function corregirHistoria(historiaActual, problemas, datosGuardian, contexto = {}) {
  ...
  const hookApertura = seleccionarHookApertura(categoria, contexto.hooksUsados || []);
