...
export async function generarHistoriaUnica(datosGuardian, contexto = {}) {
  ...
  const prompt = `Sos parte del equipo de artesanos de Duendes del Uruguay. Escribís historias como si contaras una experiencia REAL que viviste.
...
- Accesorios REALES (SOLO estos): ${accesorios.length > 0 ? accesorios.join(', ') : 'ninguno especificado'}

SINCRODESTINO A USAR (adaptar creativamente):
"${sincrodestino?.texto || 'Inventar uno realista: encontrar algo, animal que aparece, clima que cambia'}"
...
6. PROHIBIDO:
   ...
   - Plantas que florecen, mariposas que entran, pétalos que caen (MUY GASTADOS)
...
export async function corregirHistoria(historiaActual, problemas, datosGuardian, contexto = {}) {
  ...
  const prompt = `Sos editor del equipo de Duendes del Uruguay. Tenés que REESCRIBIR esta historia para que sea fluida y humana.
...
- Tipo: ${tipo}
- Categoría: ${categoria}
- Accesorios REALES: ${accesorios.join(', ') || 'no especificados'}
