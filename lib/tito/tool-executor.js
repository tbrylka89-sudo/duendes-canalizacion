...
async function escalarAHumano(input, context = {}) {
  const { motivo, detalle, urgente = false, datos_cliente = {} } = input;
  ...
  try {
    ...
    // Si es urgente, podríamos enviar notificación (TODO: integrar con WhatsApp/email)
    if (urgente) {
      console.log(`[ESCALAMIENTO URGENTE] ${ticket.id}: ${detalle}`);
      // TODO: Enviar WhatsApp al equipo
    }

    return {
      success: true,
      mensaje: 'Escalamiento registrado. El equipo se pondrá en contacto.',
      ticketId: ticket.id,
      respuestaParaCliente: urgente
        ? 'Dejame que le aviso al equipo urgente, te escriben en unos minutos 🍀'
        : 'Dejame que le aviso al equipo, te escriben enseguida 🍀'
    };
  } catch (error) {
