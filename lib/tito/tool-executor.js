...
// Buscar guardián por nombre con fuzzy matching para typos comunes
function buscarGuardianPorNombre(productos, nombre) {
  ...
  // 3. Reverse contains (input contains product name)
  encontrado = productos.find(p =>
    nombreLower.includes((p.nombre || '').toLowerCase())
  );
  if (encontrado) return encontrado;

  // 4. Fuzzy: match with 1-2 character difference (Levenshtein)
  let mejorMatch = null;
  let mejorDist = Infinity;
  for (const p of productos) {
    const pNombre = (p.nombre || '').toLowerCase();
    const dist = distanciaLevenshtein(nombreLower, pNombre);
  ...
  // 5. Slug match (slug contains the name)
  encontrado = productos.find(p =>
    (p.slug || '').includes(nombreLower.replace(/\s+/g, '-'))
  );
    ...
    return {
      ...
      productos: seleccionados.map(p => {
        const producto = {
          ...
          enOferta: p.enOferta,
          descripcion: p.descripcion?.substring(0, 300)
        };
...
async function buscarProducto({ nombre }) {
  try {
    const productos = await obtenerProductosWoo();
    const encontrado = buscarGuardianPorNombre(productos, nombre);
    ...
    return {
      ...
      producto: {
        ...
        categoria: (encontrado.categorias || []).join(', '),
...
async function obtenerGuardianCompleto({ identificador }) {
  ...
  // Try by ID first
  let guardian = productos.find(p => p.id?.toString() === identificador);
  // Then fuzzy name match
  if (!guardian) guardian = buscarGuardianPorNombre(productos, identificador);
  ...
  return {
    ...
    instruccion: "Usa esta información para hablar del guardián de forma personal y conectada. Usá el subtítulo y categoría para describir su esencia. Menciona su historia, sus dones especiales, su sincrodestino si lo tiene."
  };
}
...
async function verificarStock({ producto_id, nombre_producto }) {
  try {
    ...
    if (producto_id) {
      ...
      // Buscar en caché por nombre (fuzzy)
      const productos = await obtenerProductosWoo();
      const productoCache = buscarGuardianPorNombre(productos, nombre_producto);
      if (!productoCache) {
