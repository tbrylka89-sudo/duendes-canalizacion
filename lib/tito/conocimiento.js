...
export function recomendarGuardianes(necesidad, productos, opciones = {}) {
  ...
  if (necesidad) {
    ...
    const totalAntes = filtrados.length;
    ...
    const filtrarPorCategoria = (productos, patron) => {
      ...
      if (resultado.length > 0) {
        console.log(`[RECOM] Filtro por categorías: ${resultado.length} matches`);
        return resultado;
      }
      ...
      resultado = productos.filter(p => {
        ...
        if (match) console.log(`[RECOM] Match slug/nombre/desc: ${p.nombre} (${p.slug})`);
        return match;
      });

      console.log(`[RECOM] Filtro por slug+nombre+desc: ${resultado.length} matches de ${productos.length} productos`);
      if (resultado.length === 0) {
        // Log first 3 products' slugs for debugging
        productos.slice(0, 3).forEach(p => console.log(`[RECOM] Sample: nombre="${p.nombre}" slug="${p.slug}" cats=${JSON.stringify(p.categorias)}`));
      }
