...
import { PRECIOS_URUGUAY as CONVERTIR_UYU } from './cotizaciones';
  ...
  try {
    ...
    cacheProductos = productos
      ...
      .map(p => {
        ...
        const precioUYU = metaPrecioUYU ? parseInt(metaPrecioUYU.value) : CONVERTIR_UYU.convertir(precioUSD);

        // Obtener intenciones de la taxonomía custom (plugin duendes-tags-intencion.php)
        const intencionesAPI = p.intenciones?.map(i => i.slug) || [];
        const textoProducto = p.name + ' ' + p.slug + ' ' + (p.description || '');

        // Detectar intenciones: primero desde API, fallback a regex
        const tieneIntencion = (slug, patron) => {
          if (intencionesAPI.includes(slug)) return true;
          return patron.test(textoProducto);
        };
        ...
        return {
          ...
          // Intenciones desde taxonomía custom
          intenciones: intencionesAPI,
          // Subtítulo extraído del slug (más informativo que la descripción genérica)
          subtitulo: extraerSubtituloDeSlug(p.slug, p.name),
          ...
          // Intenciones detectadas (taxonomía API + fallback regex)
          esProteccion: tieneIntencion('proteccion', /protecci[oó]n|proteger|protector|escudo|defensa/i),
          esAbundancia: tieneIntencion('abundancia', /abundancia|prosperidad|dinero|riqueza|emprendedor|abrecaminos|fortuna|suerte/i),
          esAmor: tieneIntencion('amor', /amor|coraz[oó]n|relaci[oó]n|pareja/i),
          esSanacion: tieneIntencion('sanacion', /sanaci[oó]n|salud|sanar|bienestar|curar/i),
          esPaz: tieneIntencion('paz', /paz|calma|tranquilidad|armon[ií]a|ansiedad|estr[eé]s/i),
...
// Extraer subtítulo legible del slug (ej: "dani-para-los-dias-de-ansiedad" → "Para los días de ansiedad")
function extraerSubtituloDeSlug(slug, nombre) {
  if (!slug || !nombre) return '';
  // Remover el nombre del inicio del slug
  const nombreSlug = nombre.toLowerCase().replace(/[^a-z0-9]/g, '');
  let rest = slug;
  // Try removing name parts from slug
  const nameParts = nombre.toLowerCase().split(/[\s\-]+/);
  for (const part of nameParts) {
    if (part.length > 2) rest = rest.replace(new RegExp(`^${part}-?`), '');
  }
  rest = rest.replace(/^-+/, '');
  if (!rest || rest === slug) return '';
...
export function recomendarGuardianes(necesidad, productos, opciones = {}) {
  ...
  if (necesidad) {
    ...
    if (intencionBuscada) {
      ...
      if (resultado.length === 0) {
        ...
        const flag = flagMap[intencionBuscada];
        if (flag) {
          resultado = filtrados.filter(p => p[flag]);
        }
      }

      // 3. Si hay resultados, usarlos
      if (resultado.length > 0) {
        filtrados = resultado;
      }
    }
  ...
  // Selección con DIVERSIDAD de precios
  // Agrupar por rango de precio USD
  const rangos = {
    mini:    filtrados.filter(p => p.precio <= 75),
    pixie:   filtrados.filter(p => p.precio > 75 && p.precio <= 160),
    mediano: filtrados.filter(p => p.precio > 160 && p.precio <= 350),
    grande:  filtrados.filter(p => p.precio > 350),
  };

  // Shuffle cada rango (en oferta primero dentro de cada uno)
  Object.values(rangos).forEach(arr => {
    arr.sort((a, b) => (a.enOferta === b.enOferta ? 0 : a.enOferta ? -1 : 1));
    // Shuffle dentro de cada grupo (oferta / no oferta)
    const oferta = arr.filter(p => p.enOferta);
    const normal = arr.filter(p => !p.enOferta);
    for (let i = oferta.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [oferta[i], oferta[j]] = [oferta[j], oferta[i]];
    }
    ...
    arr.length = 0;
    arr.push(...oferta, ...normal);
  });

  // Tomar 1 de cada rango que tenga stock
  const resultado = [];
  const rangosConStock = Object.values(rangos).filter(r => r.length > 0);
  for (const rango of rangosConStock) {
    if (resultado.length >= limite) break;
    resultado.push(rango.shift());
  }
  // Llenar resto rotando rangos
...
🐉 GIGANTE - $1.050 USD
(Los Maestros Místicos son por encargo con precio especial)`
};
...
NOTA: Los Maestros Místicos son ediciones especiales con precio aparte (solo por encargo). Si alguien pregunta por un gigante, el precio es $39.800.`,
  ...
  convertir: (precioUSD) => {
    ...
    return 39800;                            // Gigante (precio máximo)
  }
};
...
export function formatearPrecio(precioUSD, codigoPais = 'US') {
  // Uruguay: usar tabla de precios FIJOS (no multiplicar por tasa)
  if (codigoPais === 'UY') {
    const precioUYU = PRECIOS_URUGUAY.convertir(precioUSD);
    return `$${precioUYU.toLocaleString('es-UY')} pesos uruguayos`;
  }
