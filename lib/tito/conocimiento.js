...
export const FAQ = {
  ...
  pagos: {
    ...
    exterior: {
      ...
      proximamente: ['Western Union', 'MoneyGram'],
    ...
    uruguay: {
      ...
      plexo: 'Transferencia bancaria vía Plexo (disponible para Uruguay y exterior)',
      nota: 'Transferencia bancaria vía Plexo disponible. Mercado Pago DESACTIVADO temporalmente.'
    },
...
- URUGUAY: Visa, MasterCard, AmEx, OCA, PassCard, Cabal, Anda, Club del Este, Redpagos, Itaú, transferencia bancaria vía Plexo
- INTERNACIONAL: Visa, MasterCard, American Express, transferencia bancaria vía Plexo

⚠️ NO TENEMOS PAYPAL - NUNCA lo menciones como opción
⚠️ MERCADO PAGO está DESACTIVADO temporalmente - no lo ofrezcas
`
  },
...
export function recomendarGuardianes(necesidad, productos, opciones = {}) {
  ...
  // 1. En oferta primero
  // 2. Precio DESCENDENTE (mostrar los más caros/mejores primero, no los mini)
  filtrados.sort((a, b) => {
    if (a.enOferta && !b.enOferta) return -1;
    if (!a.enOferta && b.enOferta) return 1;
    // Precio descendente para mostrar los mejores productos primero
