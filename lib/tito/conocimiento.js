    ...
    cacheProductos = productos
      ...
      .map(p => {
        ...
        return {
          ...
          // Extraer info útil para recomendaciones (check name + slug + description)
          esProteccion: /protecci[oó]n|proteger|protector|escudo|defensa/i.test(p.name + ' ' + p.slug + ' ' + (p.description || '')),
          esAbundancia: /abundancia|prosperidad|dinero|riqueza|emprendedor|abrecaminos|fortuna|suerte/i.test(p.name + ' ' + p.slug + ' ' + (p.description || '')),
          esAmor: /amor|coraz[oó]n|relaci[oó]n|pareja/i.test(p.name + ' ' + p.slug + ' ' + (p.description || '')),
          esSanacion: /sanaci[oó]n|salud|sanar|bienestar|curar/i.test(p.name + ' ' + p.slug + ' ' + (p.description || '')),
          esPaz: /paz|calma|tranquilidad|armon[ií]a|ansiedad|estr[eé]s/i.test(p.name + ' ' + p.slug + ' ' + (p.description || '')),
...
export function recomendarGuardianes(necesidad, productos, opciones = {}) {
  ...
  if (necesidad) {
    ...
    const filtrarPorCategoria = (productos, patron) => {
      // 1. Buscar en categorías WooCommerce (más confiable)
      ...
      // 2. Buscar en slug + nombre + descripción (todo junto para no perder matches)
      if (resultado.length === 0) {
        resultado = productos.filter(p => {
          const texto = `${p.slug || ''} ${p.nombre || ''} ${p.descripcion || ''}`.toLowerCase();
    ...
    if (/protecci[oó]n|proteger|escudo|defensa|malo|negativ/i.test(necesidadLower)) {
      filtrados = filtrarPorCategoria(filtrados, /protecci[oó]n|proteger|protector|escudo|defensa/i);
    } else if (/abundancia|dinero|prosperidad|trabajo|negocio|plata/i.test(necesidadLower)) {
      filtrados = filtrarPorCategoria(filtrados, /abundancia|prosperidad|dinero|riqueza|emprendedor|abrecaminos|fortuna|suerte/i);
    } else if (/amor|pareja|coraz[oó]n|relaci[oó]n|soledad/i.test(necesidadLower)) {
      filtrados = filtrarPorCategoria(filtrados, /amor|coraz[oó]n|pareja|relaci[oó]n/i);
    } else if (/san|salud|curar|enferm|bienestar/i.test(necesidadLower)) {
      filtrados = filtrarPorCategoria(filtrados, /sanaci[oó]n|salud|sanar|bienestar|curar/i);
    } else if (/paz|calma|ansiedad|estr[eé]s|tranquil/i.test(necesidadLower)) {
      filtrados = filtrarPorCategoria(filtrados, /paz|calma|tranquil|armon[ií]a|ansiedad|estr[eé]s/i);
    }
  }
