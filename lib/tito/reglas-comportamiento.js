...
export function detectarSpam(msg) {
  ...
  // Spam religioso / cadenas - si contiene palabras religiosas SIN intención de compra
  const tieneReligioso = /\b(am[eé]n|bendiciones?|bendecido|dios\s+te\s+bendiga|que\s+dios|la\s+virgen|gloria\s+a\s+dios|aleluya|alabado|oremos)\b/i.test(msgLower);
  const tieneIntento = /\b(precio|cu[aá]nto|guard|duende|quiero|comprar|ver|env[ií]o|tienda|protecci|abundancia)\b/i.test(msgLower);
  if (tieneReligioso && !tieneIntento && msgLower.length < 100) {
...
export function detectarDespedida(msg, tieneHistorial = false) {
  ...
  // Despedidas explícitas (pueden tener texto adicional como "chau gracias")
  if (/^(chau|cha[uo]|adi[oó]s|bye|nos\s+vemos|hasta\s+(luego|pronto|la\s+pr[oó]xima)|me\s+voy|eso\s+era\s+todo|ya\s+est[aá]|nada\s+m[aá]s)\b/i.test(msgLower)) {
...
export function detectarTrolling(msg) {
  ...
  // Caracteres repetidos (aaaaa, jjjjj)
  if (/^(.)\1{4,}$/i.test(msgLower.replace(/\s/g, ''))) {
    return { detectado: true, tipo: 'repeticion' };
  }

  // Patrones repetidos (jajaja, hahaha, jejeje, lololo)
  if (/^(.{1,3})\1{3,}$/i.test(msgLower.replace(/\s/g, ''))) {
    return { detectado: true, tipo: 'repeticion' };
  }
