...
export const TITO_TOOLS = [
  ...
  {
    name: "recomendar_guardian",
    description: "Sistema inteligente de recomendación. Analiza la conversación y recomienda guardianes personalizados. USAR cuando: 1) El cliente expresó una necesidad emocional, 2) Querés dar recomendaciones más precisas basadas en toda la conversación, 3) Detectás que puede ser cliente repetido, 4) Querés sugerir combos o guardianes complementarios. Este tool es más inteligente que mostrar_productos porque analiza el contexto completo.",
    input_schema: {
      type: "object",
      properties: {
        mensaje: {
          type: "string",
          description: "El último mensaje del cliente para analizar"
        },
        email: {
          type: "string",
          description: "Email del cliente si lo tenés (para detectar si compró antes)"
        },
        incluirCrossSell: {
          type: "boolean",
          description: "Si incluir recomendaciones de guardianes complementarios (default: true)"
        },
        incluirCombos: {
          type: "boolean",
          description: "Si incluir combos/paquetes recomendados (default: true)"
        },
        limite: {
          type: "number",
          description: "Cantidad de guardianes principales a recomendar (default: 3)"
        }
      },
      required: ["mensaje"]
    }
    ...
    input_schema: {
      ...
      properties: {
        tema: {
          ...
          enum: ["envios", "pagos", "materiales", "tamanos", "garantia", "canalizacion", "visitas", "reventa", "miMagia"]
        }
      },
      required: ["tema"]
    }
  ...
  // ─────────────────────────────────────────────────────────────
  {
    ...
    description: "Escalar la conversación al equipo humano. USAR cuando: 1) No sabés cómo responder algo, 2) El cliente tiene un problema con un pedido que no podés resolver, 3) Hay una queja o reclamo, 4) Pregunta algo muy específico que no está en tu conocimiento, 5) Pide hablar con una persona.",
    input_schema: {
      type: "object",
      properties: {
        tipo: {
          type: "string",
          description: "Tipo de escalamiento para priorizar",
          enum: ["venta", "pedido", "queja", "consulta", "otro"]
        },
        motivo: {
          type: "string",
          description: "Por qué se escala: 'quiere_comprar', 'problema_pedido', 'queja', 'pregunta_especifica', 'pide_humano', 'no_se_responder'"
        },
        detalle: {
          type: "string",
          description: "Descripción breve de qué necesita el cliente"
        },
        datos_cliente: {
          type: "object",
          description: "Datos del cliente si los tenés: nombre, email, pais, telefono",
          properties: {
            nombre: { type: "string" },
            email: { type: "string" },
            pais: { type: "string" },
